 ...
 * @returns {object|null}
 */
export function getLiturgiByDate(dateStr: string) {
  return (LITURGI_2026 as Record<string, any>)[dateStr] || null;
}
 ...
 */
export function getLiturgiMinggu(sundayDate: string) {
  const entry = (LITURGI_2026 as Record<string, any>)[sundayDate];
 ...
 * @returns {Array}
 */
export function getLiturgiByMonth(year: number, month: number) {
