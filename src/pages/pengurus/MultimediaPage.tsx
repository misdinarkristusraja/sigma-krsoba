  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center flex-wrap gap-3">
        <div>
          <h2 className="font-bold text-gray-900 dark:text-white text-base">Divisi Multimedia &amp; Konten</h2>
          <p className="text-xs text-gray-500 dark:text-slate-400">Pipeline perencanaan desain, publikasi Instagram / TikTok / YouTube.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {contentList.length === 0 ? (
          ...
          contentList.map((c) => (
            <div key={c.id} className="card p-5 border border-gray-100 dark:border-slate-800 space-y-3">
              <div className="flex items-start justify-between gap-2">
                <div>
                  <span className="text-[10px] uppercase tracking-wider font-bold bg-blue-50 dark:bg-blue-950/60 text-blue-700 dark:text-blue-300 px-2 py-0.5 rounded">
                    {c.platform}
                  </span>
                  <h3 className="font-bold text-gray-900 dark:text-white text-base mt-1">{c.judul}</h3>
                </div>
                <select
              ...
              <div className="flex items-center justify-between text-xs text-gray-500 dark:text-slate-400">
                ...
                <span>PJ: <strong className="text-gray-800 dark:text-slate-200">{c.pj?.nama_panggilan || 'Multimedia'}</strong></span>
              </div>

              {c.link_preview && (
                <a href={c.link_preview} target="_blank" rel="noreferrer" className="text-xs text-brand-800 dark:text-amber-400 hover:underline flex items-center gap-1">
      ...
      {showModal && (
        <div className="modal-overlay">
          <form onSubmit={handleAddContent} className="modal-card p-6 w-full max-w-md space-y-4">
            <h3 className="font-bold text-lg text-gray-900 dark:text-white">Tambah Konten Multimedia</h3>
            <div>
              <label className="text-xs font-semibold text-gray-700 dark:text-slate-300 mb-1 block">Judul / Topic Konten</label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs font-semibold text-gray-700 dark:text-slate-300 mb-1 block">Platform</label>
              ...
              <div>
                <label className="text-xs font-semibold text-gray-700 dark:text-slate-300 mb-1 block">Target Publish</label>
                <input type="date" className="input" value={targetDate} onChange={e => setTargetDate(e.target.value)} required />
              </div>
            </div>
            <div>
              <label className="text-xs font-semibold text-gray-700 dark:text-slate-300 mb-1 block">Status Awal</label>
            ...
            <div>
              <label className="text-xs font-semibold text-gray-700 dark:text-slate-300 mb-1 block">Link Preview / Drive (Opsional)</label>
