    ...
    <div className="space-y-6">
      ...
      {tab === 'notula' && (
        <div className="space-y-4">
          {notulaList.length === 0 ? (
            ...
            notulaList.map((n) => (
              <div key={n.id} className="card p-5 border border-gray-100 hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <h3 className="font-bold text-gray-900 text-base">{n.judul}</h3>
      ...
      {tab === 'surat' && (
        <div className="card p-0 overflow-hidden">
          <table className="tbl">
            ...
            <tbody>
              {suratList.length === 0 ? (
                <tr><td colSpan={6} className="text-center py-8 text-gray-400">Belum ada arsip surat</td></tr>
              ) : (
                suratList.map((s) => (
                  <tr key={s.id}>
                    <td className="font-mono text-xs font-bold text-gray-900">{s.nomor_surat}</td>
                    <td>
                      <span className={`badge ${s.tipe === 'Masuk' ? 'badge-blue' : 'badge-green'}`}>
                        {s.tipe}
                      </span>
                    </td>
                    <td className="text-sm font-medium text-gray-800">{s.perihal}</td>
                    <td className="text-xs text-gray-500">{s.tanggal}</td>
                    <td className="text-xs text-gray-500">{s.created_user?.nama_panggilan || '—'}</td>
                    <td>
                      {s.file_url ? (
                        <a href={s.file_url} target="_blank" rel="noreferrer" className="btn-ghost btn-xs text-brand-800 gap-1">
      ...
      {showNotulaModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <form onSubmit={handleAddNotula} className="bg-white rounded-2xl p-6 w-full max-w-lg space-y-4 shadow-2xl">
            <h3 className="font-bold text-lg text-gray-900">Catat Notula Pertemuan</h3>
            <div>
              <label className="text-xs font-semibold text-gray-700 mb-1 block">Judul / Agenda Meeting</label>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="text-xs font-semibold text-gray-700 mb-1 block">Peserta Hadir</label>
                <input className="input" placeholder="Pengurus, Pembina" value={notPeserta} onChange={e => setNotPeserta(e.target.value)} />
              </div>
            </div>
            <div>
              <label className="text-xs font-semibold text-gray-700 mb-1 block">Isi Notula / Keputusan</label>
      ...
      {showSuratModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <form onSubmit={handleAddSurat} className="bg-white rounded-2xl p-6 w-full max-w-md space-y-4 shadow-2xl">
            <h3 className="font-bold text-lg text-gray-900">Arsipkan Surat</h3>
            <div>
              <label className="text-xs font-semibold text-gray-700 mb-1 block">Nomor Surat</label>
              <input className="input" placeholder="001/MIS/07/2026" value={surNomor} onChange={e => setSurNomor(e.target.value)} required />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs font-semibold text-gray-700 mb-1 block">Tipe Surat</label>
            ...
            <div>
              <label className="text-xs font-semibold text-gray-700 mb-1 block">Perihal / Subjek</label>
            ...
            <div>
              <label className="text-xs font-semibold text-gray-700 mb-1 block">URL Dokumen / Cloud Link (Opsional)</label>
