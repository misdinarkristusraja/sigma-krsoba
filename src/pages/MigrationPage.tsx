...
function normalizeMyID(raw: any) {
  if (!raw) return null;
...
function parseTimestamp(val: any) {
...
function getCol(row: any, colName: any) {
  if (!colName) return '';
  const v = row[colName];
...
function guessChecksumCol(colNames: any[]) {
...
function guessCol(colNames: any[], hints: any[]) {
  for (const c of colNames) {
...
const FIELD_DEFS: Record<string, { key: string; label: string; required: boolean; hints: string[] }[]> = {
  members: [
...
export default function MigrationPage() {
  ...
  const [colNames,   setColNames]  = useState<any[]>([]);   // semua nama kolom dari Excel
  const [colMap,     setColMap]    = useState<Record<string,any>>({});   // mapping field → kolom Excel
  ...
  function handleFile(e: any) {
    ...
    reader.onload = (ev) => {
      try {
        const wb    = XLSX.read((ev.target as any).result, { type: 'binary', cellDates: true });
        wbRef.current = wb;
        setSheetNames(wb.SheetNames);
        const type  = MIGRATION_TYPES.find((t: any) => t.key === migType);
        const found = wb.SheetNames.find((n: any) =>
  ...
  async function runDryRun() {
    ...
    fields.filter((f: any) => f.required).forEach((f: any) => {
  ...
  async function runImport() {
    ...
    for (let i = 0; i < rawData.length; i++) {
      ...
      try {
        ...
        err++;
        errDetails.push({ row: i + 2, msg: (e as any).message, data: Object.values(row).slice(0,3).join(' | ') });
      }
    }
  ...
  // IMPORT FUNCTIONS
  // ════════════════════════════════════════════════════════════
  async function doImportMember(row: any) {
  ...
  async function doImportRegis(row: any) {
  ...
  async function doImportAbsen(row: any) {
    ...
    const typeMap: Record<string, string> = { tugas:'tugas', latihan:'latihan', 'walk-in':'walkin_tugas', walkin:'walkin_tugas' };
  ...
  async function doImportSwap(row: any) {
  ...
  async function doImportJadwal(row: any) {
    // ── 1. Ambil nilai kolom ──────────────────────────────
    const tanggalRawObj = row[colMap.tanggal];
    ...
    let tanggal = '';
    if (tanggalRawObj instanceof Date) {
      // Excel date object
      const d = tanggalRawObj;
